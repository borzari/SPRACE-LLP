...
#include "modules/ParticlePropagatorHSCP.h"
...
ParticlePropagatorHSCP::ParticlePropagatorHSCP() :
  fItInputArray(0)
{
}
...
void ParticlePropagatorHSCP::Init()
{
  ...
  fOutputArrayHSCP = ExportArray(GetString("OutputArrayHSCP", "allParticles"));
  fNeutralOutputArrayHSCP = ExportArray(GetString("NeutralOutputArrayHSCP", "neutralParticles"));
  fChargedHadronOutputArrayHSCP = ExportArray(GetString("ChargedHadronOutputArrayHSCP", "chargedHadrons"));
  fElectronOutputArrayHSCP = ExportArray(GetString("ElectronOutputArrayHSCP", "electrons"));
  fMuonOutputArrayHSCP = ExportArray(GetString("MuonOutputArrayHSCP", "muons"));
  fHSCPOutputArray = ExportArray(GetString("HSCPOutputArray", "hscp"));
}

//------------------------------------------------------------------------------

void ParticlePropagatorHSCP::Finish()
{
...
void ParticlePropagatorHSCP::Process()
{
  ...
  while((candidate = static_cast<Candidate *>(fItInputArray->Next())))
  {
    ...
    if(TMath::Hypot(x, y) > fRadius || TMath::Abs(z) > fHalfLength)
    {
      ...
      fOutputArrayHSCP->Add(candidate);
    }
    else if(TMath::Abs(q) < 1.0E-9 || TMath::Abs(fBz) < 1.0E-9)
    {
      ...
      if(TMath::Abs(q) > 1.0E-9)
      {
        ...
        {
        case 11:
          fElectronOutputArrayHSCP->Add(candidate);
          break;
        case 13:
          fMuonOutputArrayHSCP->Add(candidate);
          break;
        case 1000101:
          fHSCPOutputArray->Add(candidate);
          break;
        default:
          fChargedHadronOutputArrayHSCP->Add(candidate);
        }
      }
      else
      {
        fNeutralOutputArrayHSCP->Add(candidate);
      }
    }
    else
    {
